...
data_gen <- function(n)
{
  ...
  py <- expit(y0 + yA*A + yM2*M2 + yM1*M1 + yAM1 * A*M1 + yAM2 * A*M2 + yM1M2* M1*M2 + U %*% yU) 
...
stable1.m1 <- function(dataset,fita,M1T=T,M2T=T,YT=T,esttau)
{
  ...
  for.y <- paste0("cbind(Y,1-Y)~",paste(var.fity,collapse = "+"))
  ...
  fity.ini <- glm(paste0("cbind(Y,1-Y)~",paste(c("A","M1","M2","A*M1","A*M2","M1*M2",paste0(rep(varname.y,4),1:4)),collapse = "+")),
  ...
  while((par.converge > 0.00001 | (par.global > 0.3*abs(quad.ori))) & time <= it.max)
  {
    ...
    fity.temp <- glm(for.y,data = dataset,family = binomial(link = "logit"),weights=weighta)
  ...
  fity <- tryCatch(glm(for.y,data = dataset,family = binomial(link = "logit"),weights=weighta),
                   warning=function(w){
                     #print("final fity has "); print(w);
                     return(glm(for.y,data = dataset,family = binomial(link = "logit"),weights=weighta))})
...
ity1 <- function(alpha,beta,dataset,var.fitm1,var.fitm2,for.y,esttau)
{
  ...
  fity <- glm(for.y,data = dataset,family = binomial(link = "logit"),weights=weighta)
