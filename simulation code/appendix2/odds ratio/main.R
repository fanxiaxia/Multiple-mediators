...
Ey <- function(a,m2,m1)
{
  return(expit(y0+yA*a+yM2*m2+yM1*m1+yAM1*a*m1+yAM2*a*m2 + yM1M2*m1*m2 + yU %*% U))
}
...
corr.fun <- function(dataset.Z)
{
  ...
  fity <- glm(Y~A+U1+U2+U3+U4+M1+M2+A*M1+A*M2+M1*M2,family = binomial(link = "logit"),data = dataset.Z)
...
outcome.fun <- function(dataset.Z)
{
  ...
  fitm2.mar <- glm(M2~A+U1+U2+U3+U4,family = binomial(link = "logit"),data = dataset.Z)
  fitm1.mar <- glm(M1~A+U1+U2+U3+U4,family = binomial(link = "logit"),data = dataset.Z)
  fity <- glm(Y~A+Z1+Z2+Z3+Z4+M1+M2+A*M1+A*M2+M1*M2,family = binomial(link = "logit"),data = dataset.Z)
