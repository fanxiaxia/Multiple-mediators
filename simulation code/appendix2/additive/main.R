...
Ey <- function(a,m2,m1)
{
  return(y0+yA*a+yM2*m2+yM1*m1+ yAM1*a*m1+yAM2*a*m2 + yU %*% U)
}
...
######use the following functions to compute 
...
corr.fun <- function(dataset.Z)
{
  ...
  fity <- lm(Y~A+U1+U2+U3+U4+M1+M2+A*M1+A*M2,data = dataset.Z)
...
outcome.fun <- function(dataset.Z)
{
  ...
  fitm2.mar <- glm(M2~A+U1+U2+U3+U4,family = binomial(link = "logit"),data = dataset.Z)
  fitm1.mar <- glm(M1~A+U1+U2+U3+U4,family = binomial(link = "logit"),data = dataset.Z)
  fity <- lm(Y~A+Z1+Z2+Z3+Z4+M1+M2+A*M1+A*M2,data = dataset.Z)
